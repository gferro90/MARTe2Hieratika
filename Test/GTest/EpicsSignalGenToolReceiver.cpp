...
int main(int argc,
    ...
    if (!localCdb.Read("FirstSignalName", firstSignalName)) {
        printf("FirstSignalName not specified in configuration\n");
        return -1;
    }
    ...
    for (uint32 i = 0u; i < numberOfThreads; i++) {
        ...
        while (GetVariable(xmlFile, variable)) {
            ...
            if (start) {
                ...
                counter++;
                if ((counter >= nVarsPerThread) && (i<numberOfThreads-1u)) {
                    break;
                }
...
//for each thread create a EpicsDiodeDataSource and a HttpDiodeDataSource
    ...
    for (uint32 i = 0u; i < numberOfThreads; i++) {
        ...
        uint8 isIndependentThread=0u;
        cdb.Write("IndependentThread", isIndependentThread);
...
#else
        Vector < StreamString > functions(1u);
        functions[0] = "CopyGAM";
        functions[0].Printf("%d", i);
        functions[0].Seek(0ull);
