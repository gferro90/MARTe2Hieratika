...
void MARTe2HieratikaInterface::SetHttpExchangeTimeout(const TimeoutType &timeoutIn) {
    timeout = timeoutIn;
}
...
bool MARTe2HieratikaInterface::GetPages(const char8 *token,
                                        BufferedStreamI &response) {
...
bool MARTe2HieratikaInterface::GetPage(const char8 *pageName,
    ...
    if (ret) {
        ...
        if (ret) {
            param = "text/html,application/html+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8";
